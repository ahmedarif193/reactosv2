...
/* Write an ASCII buffer to UEFI ConOut (which may be redirected to serial) */
VOID
UefiConsoleWriteAsciiBuffer(const CHAR* Buffer, size_t Length)
{
    if (!Buffer || Length == 0) return;
    if (!GlobalSystemTable || !GlobalSystemTable->ConOut) return;

    /* Convert to UTF-16 with CRLF expansion in small chunks */
    CHAR16 Temp[384];
    size_t out = 0;
    for (size_t i = 0; i < Length; ++i)
    {
        ...
        {
            Temp[out++] = (CHAR16)(unsigned char)c;
        }

        if (out >= (sizeof(Temp)/sizeof(Temp[0])) - 2)
        {
            Temp[out] = 0;
            GlobalSystemTable->ConOut->OutputString(GlobalSystemTable->ConOut, Temp);
            out = 0;
        }
    }
    if (out)

...
UefiInitializeVideo(VOID)
{
    ...
    if (Status != EFI_SUCCESS)
    {
        TRACE("[GOP] LocateProtocol failed: %I64x\n",
    ...
    if (gop->Mode->Info)
    {
        TRACE("[GOP] Current: %ux%u, ppsl=%u, pf=%u(%s), FBBase=%p, FBSize=%I64u\n",
              ...
              (unsigned long long)(UINTN)gop->Mode->FrameBufferSize);
    }
    ...
    TRACE("[GOP] Final mode: %ux%u ppsl=%u pf=%u(%s) fb=%p size=%I64u\n",
          ...
          (unsigned long long)(UINTN)framebufferData.BufferSize);
...
{
    TRACE("Framebuffer BaseAddress       : %p\n", (PVOID)framebufferData.BaseAddress);
    TRACE("Framebuffer BufferSize        : %I64u\n",
          (unsigned long long)(UINTN)framebufferData.BufferSize);
    TRACE("Framebuffer ScreenWidth       : %u\n", (unsigned)framebufferData.ScreenWidth);
    TRACE("Framebuffer ScreenHeight      : %u\n", (unsigned)framebufferData.ScreenHeight);
    TRACE("Framebuffer PixelsPerScanLine : %u\n", (unsigned)framebufferData.PixelsPerScanLine);
    TRACE("Framebuffer PixelFormat       : %u\n", (unsigned)framebufferData.PixelFormat);
}
...
UefiVideoClearScreenColor(ULONG Color, BOOLEAN FullScreen)
{
    ...
    /* Prefer GOP Blt to fill screen when available (safer than direct VRAM) */
    if (gGop && GlobalSystemTable && GlobalSystemTable->BootServices &&
...
UefiVideoOutputChar(UCHAR Char, unsigned X, unsigned Y, ULONG FgColor, ULONG BgColor)
{
    ...
    /* Prefer GOP BLT glyph drawing when available on any arch */
    if (gGop && GlobalSystemTable && GlobalSystemTable->BootServices)
    {
...
UefiVideoScrollUp(VOID)
{
    ...
    /* Prefer GOP blt scroll when available */
    if (gGop && GlobalSystemTable && GlobalSystemTable->BootServices &&
        framebufferData.ScreenWidth != 0 && framebufferData.ScreenHeight != 0)
    {

...
#if defined(_M_AMD64)
#include <arch/amd64/amd64_uefi.h>
#endif
                      ...
                      PCSTR BootPath)
{
    ...
    TRACE("[WINLDR] Loading boot-start drivers (BootPath='%s')\n",
          BootPath ? BootPath : "<null>");
    ...
    TRACE("[WINLDR] Boot-start driver pass complete (status=%s)\n",
          ret ? "success" : "failure");
    return ret;
}
                 ...
                 TYPE_OF_MEMORY MemoryType)
{
    ...
    *Size = 0;
    ...
    TRACE("[WINLDR] WinLdrLoadModule('%s', type=%lu)\n",
          ModuleName ? ModuleName : "<null>", (ULONG)MemoryType);
    ...
    *Size = FileSize;
    ...
    TRACE("[WINLDR] Loaded module '%s' at %p (size=0x%x)\n",
 ...
 ****/
...
#if defined(_M_AMD64)
    if (ArchInterface.ArchPrepareForKernel)
    {
        KERNEL_PARAMS Params;
        ULONG MapCount = 0;

        RtlZeroMemory(&Params, sizeof(Params));
        Params.LoaderBlock = LoaderBlockVA;
        Params.SystemTable = GlobalSystemTable;
        Params.MemoryMap = Amd64GetFreeldrMemoryMap(&MapCount);
        Params.MemoryMapCount = MapCount;
        Params.KernelEntry = (ULONG64)(ULONG_PTR)KiSystemStartup;

        EFI_STATUS Status = ArchInterface.ArchPrepareForKernel(&Params);
        if (EFI_ERROR(Status))
        {
            ERR("ArchPrepareForKernel failed: %I64x\n",
    ...
    /* Save final value of LoaderPagesSpanned */
    {
        ...
        ULONGLONG KernelImageBaseVA = 0;
        PLOADER_PARAMETER_EXTENSION ExtensionPA = NULL;

        if (KernelDTE && KernelDTE->DllBase)
        {
            KernelImageBaseVA = (ULONGLONG)(ULONG_PTR)KernelDTE->DllBase;
        }
        ...
        if (ExtensionVA != NULL)
        {
            ...
            if (ExtAddr >= ARM64_KSEG0_BASE)
            {
                ...
                if (ExtensionPA != NULL)
                {
                    ...
                    ExtensionPA->KernelImageBase = KernelImageBaseVA;
                    __asm__ __volatile__("dsb sy" ::: "memory");
                    TRACE("ARM64: LoaderPagesSpanned written via PA (safer)\n");
                    if (KernelImageBaseVA != 0)
                    {
                        TRACE("ARM64: KernelImageBase written via PA: 0x%llx\n",
                              (unsigned long long)KernelImageBaseVA);
                    }
            ...
            else
            {
                ...
                ExtensionVA->KernelImageBase = KernelImageBaseVA;
                __asm__ __volatile__("dsb sy" ::: "memory");
                TRACE("ARM64: LoaderPagesSpanned written to PA=0x%p\n", (PVOID)ExtensionVA);
                if (KernelImageBaseVA != 0)
                {
                    TRACE("ARM64: KernelImageBase written directly: 0x%llx\n",
                          (unsigned long long)KernelImageBaseVA);
                }
            }
        }
        ...
        if (KernelImageBaseVA != 0)
        {
            TRACE("ARM64: KernelImageBaseVA final = 0x%llx\n",
                  (unsigned long long)KernelImageBaseVA);
        }
#else
        ...
        if (KernelDTE && KernelDTE->DllBase)
        {
            LoaderBlock->Extension->KernelImageBase = (ULONGLONG)(ULONG_PTR)KernelDTE->DllBase;
        }
...
#if defined(_M_AMD64)
    if (ArchInterface.ArchJumpToKernel)
    {
        ArchInterface.ArchJumpToKernel((PVOID)KiSystemStartup, LoaderBlockVA);
    }
    else
    {

...
MempAllocatePageTables(VOID)
{
    ...
    if (!PxeBase)
    {
        DbgPrint("failed to allocate PML4\n");
        return FALSE;
    }
...
MempMapRangeOfPages(ULONG64 VirtualAddress, ULONG64 PhysicalAddress, PFN_NUMBER cPages)
{
    ...
    for (i = 0; i < cPages; i++)
    {
        ...
        {
            DbgPrint("Failed to map page %ld from %p to %p\n",
...
{
    TRACE(">>> MempSetupPaging(0x%I64x, %I64u, %p)\n",
            (unsigned long long)(ULONG_PTR)StartPage,
            (unsigned long long)(ULONG_PTR)NumberOfPages,
            (PVOID)(ULONG_PTR)(StartPage * PAGE_SIZE + KSEG0_BASE));
    ...
    {
        DbgPrint("Failed to map pages %I64u, %I64u\n",
                 (unsigned long long)(ULONG_PTR)StartPage,
                 (unsigned long long)(ULONG_PTR)NumberOfPages);
        return FALSE;
    }
...
WinLdrMapSpecialPages(VOID)
{
    ...
    {
        DbgPrint("Could not map KI_USER_SHARED_DATA\n");
        return FALSE;
    }

 ...
 *  Updated: make heap independent of global LUT symbols
...
/* --- NEW: LUT getters (no global PageLookupTableAddress dependencies) --- */
extern PVOID      FrLdrMmGetLookupTableAddress(void);
/* (Total pages getter is not needed here, so we don’t declare it) */
    ...
    TYPE_OF_MEMORY MemoryType)
{
    ...
    while (Remaining > 1)
    {
        ...
        /* Subtract current block size from remainder */
    ...
    PVOID HeapHandle)
{
    ...
    {
        PVOID Lut = FrLdrMmGetLookupTableAddress();
        if (Lut)
        {
    ...
    RtlFillMemory(Heap, Heap->MaximumSize, 0xCC);
#endif /* DBG */

#ifdef FREELDR_HEAP_VERIFIER
    /* Walk all blocks and check the integrity */
    ...
    PVOID HeapHandle)
{
         ...
         Block = Block + 1 + Block->Size)
    {
        ...
        if (EndAddress > StartAddress)
        {
            ...
            /* Now mark the pages free in the LUT */
            PVOID Lut = FrLdrMmGetLookupTableAddress();
            if (Lut)
            {
                MmMarkPagesInLookupTable(
    ...
    TRACE("HeapRelease() done, freed %lu of %lu pages\n",
          AllFreePages, (unsigned long)(Heap->MaximumSize / MM_PAGE_SIZE));
}
...
#endif
        /* FIXME ! HUGE BUG : DELETING this trace causes an exception ! (timing issue ?) */
        TRACE("HeapAllocate(%p, %ld, %.4s) -> return %p\n",
              HeapHandle, ByteSize, (char*)&Tag, Block->Data);
...
#if DBG
    /* Erase contents */
    RtlFillMemory(Block->Data, Block->Size * sizeof(HEAP_BLOCK), 0xCC);
#endif
    ...
    IN SIZE_T Size)
{
    ...
    PVOID ptr = FrLdrHeapAllocateEx(FrLdrDefaultHeap, Size, ' ltR');

    ...
    OUT PLDR_DATA_TABLE_ENTRY *NewEntry)
{
    ...
    USHORT Length;
    static BOOLEAN LayoutDumped = FALSE;
    ...
    if (!LayoutDumped)
    {
        TRACE("ARM64: sizeof(LDR_DATA_TABLE_ENTRY)=0x%llx DllBase offset=0x%llx\n",
              (unsigned long long)sizeof(LDR_DATA_TABLE_ENTRY),
              (unsigned long long)FIELD_OFFSET(LDR_DATA_TABLE_ENTRY, DllBase));
        LayoutDumped = TRUE;
    }
#endif
...
#if defined(_M_ARM64)
    TRACE("ARM64: DllBase set to %p (PA %p) SizeOfImage=0x%lx\n",
          DataTableEntry->DllBase,
          BasePA,

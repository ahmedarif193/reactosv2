...
BOOLEAN CacheInitializeDrive(UCHAR DriveNumber)
{
    ...
    if (!UefiDiskGetDriveGeometry(DriveNumber, &DriveGeometry))
    {
        return FALSE;
    }
    ...
    CacheManagerDrive.BlockSize = UefiDiskGetCacheableBlockCount(DriveNumber);
...
BOOLEAN CacheReadDiskSectors(UCHAR DiskNumber, ULONGLONG StartSector, ULONG SectorCount, PVOID Buffer)
{
    ...
    if (BlockCount > 0)
    {
        ...
        //
        RtlCopyMemory(Buffer, (PVOID)((ULONG_PTR)CacheBlock->BlockData + (SectorOffsetInStartBlock * CacheManagerDrive.BytesPerSector)), (CopyLengthInStartBlock * CacheManagerDrive.BytesPerSector));
    ...
    for (Idx=StartBlock+1; BlockCount>1; Idx++)
    {
        ...
        // Copy the portion requested into the buffer
        //
        RtlCopyMemory(Buffer, CacheBlock->BlockData, CacheManagerDrive.BlockSize * CacheManagerDrive.BytesPerSector);
    ...
    if (BlockCount > 0)
    {
        ...
        // Copy the portion requested into the buffer
        //
        RtlCopyMemory(Buffer, CacheBlock->BlockData, SectorOffsetInEndBlock * CacheManagerDrive.BytesPerSector);

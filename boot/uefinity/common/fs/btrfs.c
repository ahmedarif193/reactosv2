                                    ...
                                    u64 offset, u64 size, char *out)
{
    ...
    if (extent->compression == BTRFS_COMPRESS_NONE)
    {
        ...
        RtlCopyMemory(out, cbuf + offset, size);
        return size;
    }
                                 ...
                                 u64 offset, u64 size, char *out)
{
    ...
    if (extent->compression == BTRFS_COMPRESS_NONE)
    {
        ...
        if (physical & (SECTOR_SIZE - 1))
        {
            ...
            {
                RtlCopyMemory(out, temp_out + shift, size);
                FrLdrTempFree(temp_out, TAG_BTRFS_FILE);
                return size;
            }

            RtlCopyMemory(out, temp_out + shift, SECTOR_SIZE - shift);
            FrLdrTempFree(temp_out, TAG_BTRFS_FILE);
                              ...
                              u64 inr, char **target)
{
    ...
    *target = FrLdrTempAlloc(extent->ram_bytes + 1, TAG_BTRFS_LINK);
    ...
    RtlCopyMemory(*target, data_ptr, extent->ram_bytes);

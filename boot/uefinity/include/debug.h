...
/* Early quiet logging control: buffer debug output and flush later */
VOID    DebugSetEarlyQuiet(BOOLEAN Enable);
VOID    DebugFlushEarlyLog(VOID);

size_t  DebugLogSnapshot(const CHAR*** LinesOut);
...
void    MEMORY_WRITE_BREAKPOINT4(unsigned long addr);
    ...
    #define ERR_CH(ch, fmt, ...)   CRITICAL_TRACE("[ERR] " fmt, ##__VA_ARGS__)
    #define FIXME_CH(ch, fmt, ...) CRITICAL_TRACE("[FIXME] " fmt, ##__VA_ARGS__)
    #define WARN_CH(ch, fmt, ...)  CRITICAL_TRACE("[WARN] " fmt, ##__VA_ARGS__)
    #define TRACE_CH(ch, fmt, ...) CRITICAL_TRACE("[TRACE] " fmt, ##__VA_ARGS__)

    #define ERR(fmt, ...)          CRITICAL_TRACE("[ERR] " fmt, ##__VA_ARGS__)
    #define FIXME(fmt, ...)        CRITICAL_TRACE("[FIXME] " fmt, ##__VA_ARGS__)
    #define WARN(fmt, ...)         CRITICAL_TRACE("[WARN] " fmt, ##__VA_ARGS__)
    /* AGENT-MODIFIED: Route TRACE to the critical logger in Release builds */
    #define TRACE(fmt, ...)        CRITICAL_TRACE("[TRACE] " fmt, ##__VA_ARGS__)

...
static HRESULT reg_get_typelib_module(REFIID iid, WCHAR *module, DWORD len)
{
    ...
    snprintf( interfacekey, sizeof(interfacekey), "Interface\\{%08x-%04x-%04x-%02x%02x-%02x%02x%02x%02x%02x%02x}\\Typelib",
    ...
    {
#ifdef _WIN64
        snprintf(typelibkey, sizeof(typelibkey), "Typelib\\%s\\%s\\0\\win32", tlguid, ver);
        tlfnlen = sizeof(tlfn);
        if (RegQueryValueA(HKEY_CLASSES_ROOT, typelibkey, tlfn, &tlfnlen))
        {
#endif

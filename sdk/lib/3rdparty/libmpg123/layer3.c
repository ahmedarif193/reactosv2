...
static int III_dequantize_sample(mpg123_handle *fr, real xr[SBLIMIT][SSLIMIT],int *scf, struct gr_info_s *gr_info,int sfreq,int part2bits)
{
    ...
    /* Treat the 2D xr buffer as a flat array of SBLIMIT*SSLIMIT reals. */
    real *xrpnt = &xr[0][0];
    real *xr_end = &xr[0][0] + (SBLIMIT * SSLIMIT);
	int l[3],l3;
...
#define CHECK_XRPNT if(xrpnt >= xr_end) \
{ \
    if(NOQUIET) \
        error2("attempted xrpnt overflow (%p !< %p)", (void*) xrpnt, (void*) xr_end); \
    return 1; \
}

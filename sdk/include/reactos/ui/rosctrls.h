...
public:
    ...
    {
        SendMessage(m_hWnd, WM_SETREDRAW, redraw, 0);
    }
    ...
    int InsertColumn(int iCol, LV_COLUMNW* pcol)
    {
        return (int)SendMessage(LVM_INSERTCOLUMNW, iCol, reinterpret_cast<LPARAM>(pcol));
    }
    ...
    {
        return (int)SendMessage(LVM_INSERTITEMW, 0, reinterpret_cast<LPARAM>(pitem));
    }
    ...
    BOOL GetItem(LV_ITEMW* pitem)
    {
        return (BOOL)SendMessage(LVM_GETITEMW, 0, reinterpret_cast<LPARAM>(pitem));
    }

    BOOL SetItem(const LV_ITEMW * pitem)
    {
        return (BOOL)SendMessage(LVM_SETITEMW, 0, reinterpret_cast<LPARAM>(pitem));
    }

    BOOL FindItem(int iStart, const LV_FINDINFOW * plvfi)
    {
        return (BOOL)SendMessage(LVM_FINDITEMW, iStart, (LPARAM) plvfi);
    }
    ...
    int HitTest(LVHITTESTINFO * phtInfo)
    {
    ...
    BOOL GetSelectedItem(LV_ITEMW* pItem)
    {
...
public: // Configuration methods
    ...
    {
        return SendMessageW(m_hWnd, TB_BUTTONSTRUCTSIZE, sizeof(TBBUTTON), 0);
    }

    HWND GetTooltip()
    {
        return (HWND)SendMessageW(m_hWnd, TB_GETTOOLTIPS, 0, 0);
    }

    DWORD SetTooltip(HWND hWndTooltip)
    {
        return SendMessageW(m_hWnd, TB_SETTOOLTIPS, reinterpret_cast<WPARAM>(hWndTooltip), 0);
    }

    INT GetHotItem()
    {
        return SendMessageW(m_hWnd, TB_GETHOTITEM, 0, 0);
    }

    DWORD SetHotItem(INT item)
    {
        return SendMessageW(m_hWnd, TB_SETHOTITEM, item, 0);
    }

    DWORD SetDrawTextFlags(DWORD useBits, DWORD bitState)
    {
        return SendMessageW(m_hWnd, TB_SETDRAWTEXTFLAGS, useBits, bitState);
    }
...
public: // Button list management methods
    ...
    {
        return SendMessageW(m_hWnd, TB_GETBUTTON, index, reinterpret_cast<LPARAM>(btn));
    }

    DWORD AddButton(TBBUTTON * btn)
    {
        return SendMessageW(m_hWnd, TB_ADDBUTTONS, 1, reinterpret_cast<LPARAM>(btn));
    }

    DWORD AddButtons(int count, TBBUTTON * buttons)
    {
        return SendMessageW(m_hWnd, TB_ADDBUTTONS, count, reinterpret_cast<LPARAM>(buttons));
    }

    DWORD InsertButton(int insertAt, TBBUTTON * btn)
    {
        return SendMessageW(m_hWnd, TB_INSERTBUTTON, insertAt, reinterpret_cast<LPARAM>(btn));
    }

    DWORD MoveButton(int oldIndex, int newIndex)
    {
        return SendMessageW(m_hWnd, TB_MOVEBUTTON, oldIndex, newIndex);
    }

    DWORD DeleteButton(int index)
    {
        return SendMessageW(m_hWnd, TB_DELETEBUTTON, index, 0);
    }

    DWORD GetButtonInfo(int cmdId, TBBUTTONINFO * info)
    {
        return SendMessageW(m_hWnd, TB_GETBUTTONINFO, cmdId, reinterpret_cast<LPARAM>(info));
    }

    DWORD SetButtonInfo(int cmdId, TBBUTTONINFO * info)
    {
        return SendMessageW(m_hWnd, TB_SETBUTTONINFO, cmdId, reinterpret_cast<LPARAM>(info));
    }

    DWORD CheckButton(int cmdId, BOOL bCheck)
    {
        return SendMessageW(m_hWnd, TB_CHECKBUTTON, cmdId, MAKELPARAM(bCheck, 0));
    }

public: // Layout management methods
    DWORD GetButtonSize()
    {
        return SendMessageW(m_hWnd, TB_GETBUTTONSIZE, 0, 0);
    }

    DWORD SetButtonSize(int w, int h)
    {
        return SendMessageW(m_hWnd, TB_SETBUTTONSIZE, 0, MAKELPARAM(w, h));
    }

    DWORD AutoSize()
    {
        return SendMessageW(m_hWnd, TB_AUTOSIZE, 0, 0);
    }

    DWORD GetMaxSize(LPSIZE size)
    {
        return SendMessageW(m_hWnd, TB_GETMAXSIZE, 0, reinterpret_cast<LPARAM>(size));
    }

    DWORD GetIdealSize(BOOL useHeight, LPSIZE size)
    {
        return SendMessageW(m_hWnd, TB_GETIDEALSIZE, useHeight, reinterpret_cast<LPARAM>(size));
    }

    DWORD GetMetrics(TBMETRICS * tbm)
    {
        return SendMessageW(m_hWnd, TB_GETMETRICS, 0, reinterpret_cast<LPARAM>(tbm));
    }

    DWORD SetMetrics(TBMETRICS * tbm)
    {
        return SendMessageW(m_hWnd, TB_SETMETRICS, 0, reinterpret_cast<LPARAM>(tbm));
    }

    DWORD GetItemRect(int index, LPRECT prcItem)
    {
        return SendMessageW(m_hWnd, TB_GETITEMRECT, index, reinterpret_cast<LPARAM>(prcItem));
    }

    DWORD SetRedraw(BOOL bEnable)
    {
        return SendMessageW(m_hWnd, WM_SETREDRAW, bEnable, 0);
    }

    DWORD GetPadding()
    {
        return SendMessageW(m_hWnd, TB_GETPADDING, 0, 0);
    }

    DWORD SetPadding(int x, int y)
    {
        return SendMessageW(m_hWnd, TB_SETPADDING, 0, MAKELPARAM(x, y));
    }

public: // Image list management methods
    HIMAGELIST SetImageList(HIMAGELIST himl)
    {
        return (HIMAGELIST)SendMessageW(m_hWnd, TB_SETIMAGELIST, 0, reinterpret_cast<LPARAM>(himl));
    }

public: // Other methods
    INT HitTest(PPOINT ppt)
    {
        return (INT) SendMessageW(m_hWnd, TB_HITTEST, 0, reinterpret_cast<LPARAM>(ppt));
    }
...
public:
    ...
    BOOL SetBkColor(COLORREF cr)
    {
        return (BOOL) SendMessage(m_hWnd, TVM_SETBKCOLOR, 0, cr);
    }

    BOOL SetTextColor(COLORREF cr)
    {
        return (BOOL) SendMessage(m_hWnd, TVM_SETTEXTCOLOR, 0, cr);
    }

    HIMAGELIST SetImageList(HIMAGELIST himl, int iImageList)
    {
        return (HIMAGELIST) SendMessage(m_hWnd, TVM_SETIMAGELIST, iImageList, reinterpret_cast<LPARAM>(himl));
    }

    HTREEITEM InsertItem(const TVINSERTSTRUCTW * pitem)
    {
        return (HTREEITEM) SendMessage(m_hWnd, TVM_INSERTITEM, 0, reinterpret_cast<LPARAM>(pitem));
    }

    BOOL DeleteItem(HTREEITEM i)
    {
        return (BOOL) SendMessage(m_hWnd, TVM_DELETEITEM, 0, (LPARAM)i);
    }

    BOOL GetItem(TV_ITEM* pitem)
    {
        return (BOOL) SendMessage(m_hWnd, TVM_GETITEM, 0, reinterpret_cast<LPARAM>(pitem));
    }

    BOOL SetItem(const TV_ITEM * pitem)
    {
        return (BOOL) SendMessage(m_hWnd, TVM_SETITEM, 0, reinterpret_cast<LPARAM>(pitem));
    }
    ...
    {
        return (BOOL) SendMessage(m_hWnd, TVM_ENSUREVISIBLE, 0, (LPARAM)i);
    }

    HWND EditLabel(HTREEITEM i)
    {
        return (HWND) SendMessage(m_hWnd, TVM_EDITLABEL, 0, (LPARAM)i);
    }

    HTREEITEM GetNextItem(HTREEITEM i, WORD flags)
    {
        return (HTREEITEM)SendMessage(m_hWnd, TVM_GETNEXTITEM, flags, (LPARAM)i);
    }

    UINT GetItemState(int i, UINT mask)
    {
        return SendMessage(m_hWnd, TVM_GETITEMSTATE, i, (LPARAM) mask);
    }

    HTREEITEM HitTest(TVHITTESTINFO * phtInfo)
    {
        return (HTREEITEM) SendMessage(m_hWnd, TVM_HITTEST, 0, reinterpret_cast<LPARAM>(phtInfo));
    }
    ...
    DWORD_PTR GetItemData(HTREEITEM item)
    {
        ...
        BOOL ret = GetItem((TV_ITEM*)&lvItem);
        return (DWORD_PTR) (ret ? lvItem.lParam : NULL);
    }
    ...
    {
        return SendMessage(m_hWnd, TVM_EXPAND, action, (LPARAM)item);
    }

    BOOL SelectItem(HTREEITEM item, DWORD action = TVGN_CARET)
    {
        return SendMessage(m_hWnd, TVM_SELECTITEM, action, (LPARAM) item);
    }
...
public: // Configuration methods
    ...
    {
        RECT r = { 0 };
        return CWindow::Create(TOOLTIPS_CLASSW, hWndParent, r, L"", dwStyles, dwExStyles);
    }
...
public: // Relay event
    ...
    {
        SendMessageW(m_hWnd, TTM_RELAYEVENT, extraInfo, reinterpret_cast<LPARAM>(pMsg));
    }
...
public: // Helpers
    ...
    INT GetToolCount()
    {
        return SendMessageW(m_hWnd, TTM_GETTOOLCOUNT, 0, 0);
    }

    BOOL AddTool(IN CONST TTTOOLINFOW * pInfo)
    {
        return SendMessageW(m_hWnd, TTM_ADDTOOL, 0, reinterpret_cast<LPARAM>(pInfo));
    }
    ...
    VOID DelTool(IN HWND hwndToolOwner, IN UINT uId)
    {
        ...
        info.uId = uId;
        SendMessageW(m_hWnd, TTM_DELTOOL, 0, reinterpret_cast<LPARAM>(&info));
    }
    ...
    VOID NewToolRect(IN HWND hwndToolOwner, IN UINT uId, IN RECT rect)
    {
        ...
        info.uId = uId;
        info.rect = rect;
        SendMessageW(m_hWnd, TTM_NEWTOOLRECT, 0, reinterpret_cast<LPARAM>(&info));
    }
    ...
    BOOL GetToolInfo(IN HWND hwndToolOwner, IN UINT uId, IN OUT TTTOOLINFOW * pInfo)
    {
        ...
        pInfo->uId = uId;
        return SendMessageW(m_hWnd, TTM_GETTOOLINFO, 0, reinterpret_cast<LPARAM>(pInfo));
    }

    VOID SetToolInfo(IN CONST TTTOOLINFOW * pInfo)
    {
        SendMessageW(m_hWnd, TTM_SETTOOLINFO, 0, reinterpret_cast<LPARAM>(pInfo));
    }

    BOOL HitTest(IN CONST TTHITTESTINFOW * pInfo)
    {
        return SendMessageW(m_hWnd, TTM_HITTEST, 0, reinterpret_cast<LPARAM>(pInfo));
    }
    ...
    VOID GetText(IN HWND hwndToolOwner, IN UINT uId, OUT PWSTR pBuffer, IN DWORD cchBuffer)
    {
        ...
        SendMessageW(m_hWnd, TTM_GETTEXT, cchBuffer, reinterpret_cast<LPARAM>(&info));
    }
    ...
    VOID UpdateTipText(IN HWND hwndToolOwner, IN UINT uId, IN PCWSTR szText, IN HINSTANCE hinstResourceOwner = NULL)
    {
        ...
        SendMessageW(m_hWnd, TTM_UPDATETIPTEXT, 0, reinterpret_cast<LPARAM>(&info));
    }

    BOOL EnumTools(IN CONST TTTOOLINFOW * pInfo)
    {
        return SendMessageW(m_hWnd, TTM_ENUMTOOLS, 0, reinterpret_cast<LPARAM>(pInfo));
    }

    BOOL GetCurrentTool(OUT OPTIONAL TTTOOLINFOW * pInfo = NULL)
    {
        return SendMessageW(m_hWnd, TTM_GETCURRENTTOOL, 0, reinterpret_cast<LPARAM>(pInfo));
    }

    VOID GetTitle(TTGETTITLE * pTitleInfo)
    {
        SendMessageW(m_hWnd, TTM_GETTITLE, 0, reinterpret_cast<LPARAM>(pTitleInfo));
    }

    BOOL SetTitle(PCWSTR szTitleText, WPARAM icon = 0)
    {
        return SendMessageW(m_hWnd, TTM_SETTITLE, icon, reinterpret_cast<LPARAM>(szTitleText));
    }
    ...
    VOID TrackActivate(IN HWND hwndToolOwner, IN UINT uId)
    {
        ...
        info.hwnd = hwndToolOwner;
        info.uId = uId;
        SendMessageW(m_hWnd, TTM_TRACKACTIVATE, TRUE, reinterpret_cast<LPARAM>(&info));
    }

    VOID TrackDeactivate()
    {
        SendMessageW(m_hWnd, TTM_TRACKACTIVATE, FALSE, NULL);
    }

    VOID TrackPosition(IN WORD x, IN WORD y)
    {
        SendMessageW(m_hWnd, TTM_TRACKPOSITION, 0, MAKELPARAM(x, y));
    }

    // Opens the tooltip
    VOID Popup()
    {
        SendMessageW(m_hWnd, TTM_POPUP, 0, 0);
    }

    // Closes the tooltip - Pressing the [X] for a TTF_CLOSE balloon is equivalent to calling this
    VOID Pop()
    {
        SendMessageW(m_hWnd, TTM_POP, 0, 0);
    }

    // Delay times for AUTOMATIC tooltips (they don't affect balloons)
    INT GetDelayTime(UINT which)
    {
        return SendMessageW(m_hWnd, TTM_GETDELAYTIME, which, 0);
    }

    VOID SetDelayTime(UINT which, WORD time)
    {
        SendMessageW(m_hWnd, TTM_SETDELAYTIME, which, MAKELPARAM(time, 0));
    }

    // Activates or deactivates the automatic tooltip display when hovering a control
    VOID Activate(IN BOOL bActivate = TRUE)
    {
        SendMessageW(m_hWnd, TTM_ACTIVATE, bActivate, 0);
    }
    ...
    {
        SendMessageW(m_hWnd, TTM_ADJUSTRECT, bTextToWindow, reinterpret_cast<LPARAM>(pRect));
    }

    // Useful for TTF_ABSOLUTE|TTF_TRACK tooltip positioning
    SIZE GetBubbleSize(IN TTTOOLINFOW * pInfo)
    {
        DWORD ret = SendMessageW(m_hWnd, TTM_GETBUBBLESIZE, 0, reinterpret_cast<LPARAM>(pInfo));
    ...
    {
        SendMessageW(m_hWnd, TTM_GETMARGIN, 0, reinterpret_cast<LPARAM>(pRect));
    }

    VOID SetMargin(IN RECT * pRect)
    {
        SendMessageW(m_hWnd, TTM_SETMARGIN, 0, reinterpret_cast<LPARAM>(pRect));
    }

    // Gets a previously established max width. Returns -1 if no limit is set
    INT GetMaxTipWidth()
    {
        return SendMessageW(m_hWnd, TTM_GETMAXTIPWIDTH, 0, 0);
    }

    INT SetMaxTipWidth(IN OPTIONAL INT width = -1)
    {
        return SendMessageW(m_hWnd, TTM_SETMAXTIPWIDTH, 0, width);
    }

    // Get the color of the tooltip text
    COLORREF GetTipTextColor()
    {
        return SendMessageW(m_hWnd, TTM_GETTIPTEXTCOLOR, 0, 0);
    }

    VOID SetTipTextColor(IN COLORREF textColor)
    {
        SendMessageW(m_hWnd, TTM_SETTIPTEXTCOLOR, textColor, 0);
    }

    COLORREF GetTipBkColor()
    {
        return SendMessageW(m_hWnd, TTM_GETTIPBKCOLOR, 0, 0);
    }

    VOID SetTipBkColor(IN COLORREF textColor)
    {
        SendMessageW(m_hWnd, TTM_SETTIPBKCOLOR, textColor, 0);
    }

    VOID SetWindowTheme(IN PCWSTR szThemeName)
    {
        SendMessageW(m_hWnd, TTM_SETWINDOWTHEME, 0, reinterpret_cast<LPARAM>(szThemeName));
    }
    ...
    {
        return reinterpret_cast<HWND>(SendMessageW(m_hWnd, TTM_WINDOWFROMPOINT, 0, reinterpret_cast<LPARAM>(pPoint)));
    }
};

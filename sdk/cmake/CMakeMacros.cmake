...
macro(dir_to_num dir var)
    ...
    elseif(${dir} STREQUAL reactos/3rdParty)
        set(${var} 63)
    elseif(${dir} STREQUAL reactos/Custom)
        set(${var} 83)
...
function(add_cd_file)
    ...
    if(NOT __cd EQUAL -1)
        ...
        list(FIND _CD_FOR liveimg __bootcd_liveimg_index)
        if(__bootcd_liveimg_index EQUAL -1)
            string(REGEX MATCH "^reactos(/|$)" __bootcd_liveimg_dest "${_CD_DESTINATION}")
            if(__bootcd_liveimg_dest)
                foreach(item ${_CD_FILE})
                    if(_CD_NAME_ON_CD)
                        set(__file ${_CD_NAME_ON_CD})
                    else()
                    ...
                    if(__file STREQUAL "reactos.cab")
                        continue()
                    endif()
...
function(get_includes OUTPUT_VAR)
    ...
    get_directory_property(_system_includes SYSTEM_INCLUDE_DIRECTORIES)
    foreach(arg ${_includes} ${_system_includes})
        # Skip generator expressions; custom tools can't evaluate them.
        if (NOT arg MATCHES [[^\$<.*>$]])

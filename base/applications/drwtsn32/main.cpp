...
static void PrintThread(FILE* output, DumpData& data, DWORD tid, ThreadData& thread)
{
    ...
    if ((ctx.ContextFlags & CONTEXT_INTEGER) == CONTEXT_INTEGER)
    {
        ...
        xfprintf(output, "x24:%p x25:%p x26:%p x27:%p x28:%p fp:%p lr:%p" NEWLINE,
                 ctx.X24, ctx.X25, ctx.X26, ctx.X27, ctx.X28, ctx.Fp, ctx.Lr);
#else
#error Unknown architecture
    ...
    if ((ctx.ContextFlags & CONTEXT_CONTROL) == CONTEXT_CONTROL)
    {
        ...
        // ARM64 CONTEXT structure might have different field names
        xfprintf(output, "sp:%p pc:%p" NEWLINE,
                 (void*)ctx.Sp, (void*)ctx.Pc);
#else
#error Unknown architecture
#endif
    }
